...
public class OrderController {
    ...
    public String cancelOrder(@RequestParam Integer orderId, @RequestParam String address) throws Throwable {
        SmartContract loyalContract = new SmartContract(new Hash160(config.getLoyalContract()), neow3j);

        InvocationResult res = loyalContract.callInvokeFunction("cancelOrder", List.of(ContractParameter.integer(orderId)),
        ...
        NeoSendRawTransaction tx = loyalContract.invokeFunction("cancelOrder",
                        ContractParameter.integer(orderId))
