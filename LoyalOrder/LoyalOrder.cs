...
namespace LoyalOrder
{
    ...
    public class LoyalOrder : SmartContract
    {
        ...
        private const byte Prefix_UserOrder = 0x01;
        private static StorageMap Orders => new(Storage.CurrentContext, Prefix_Order);
        private static StorageMap UserOrders => new(Storage.CurrentContext, Prefix_UserOrder);
        ...
        public static bool Order(UInt160 fromAssetHash, UInt160 toAssetHash, UInt160 fromAddress, BigInteger amount)
        {
            ...
            foreach (var (key, value) in iterator)
            {
                ...
                if (UserOrders.Get((ByteString)toOrderId) == null)
                {
                ...
                if (amount >= curAmount)
                {
                    ...
                    Orders.Delete(newKey);
                    UserOrders.Delete((ByteString)toOrderId);
                    if (amount > curAmount)
                    {
                    ...
                    else 
                    {
                        ...
                        UserOrders.Delete((ByteString)fromOrderId);
                    }
        ...
        public static bool CancelOrder(BigInteger orderId)
        {
            UInt160 user = (UInt160)UserOrders.Get((ByteString)orderId);
            Assert(Runtime.CheckWitness(user), "Not authorized.");
            UserOrders.Delete((ByteString)orderId);
            return true;
        ...
        private static byte[] CreateOrder(UInt160 fromAssetHash, UInt160 toAssetHash, UInt160 userAddress, BigInteger amount)
        {
            ...
            Orders.Put(tokenKey, newValue);
            UserOrders.Put((ByteString)orderId, userAddress);
